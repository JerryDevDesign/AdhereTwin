...
export default function PatientDashboard() {
  ...
  if (!patientId || !data) {
    return (
      <main style={{ minHeight: '100vh', background: '#090d16', color: '#f8fafc', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20, fontFamily: 'system-ui' }}>
        <div style={{ background: '#111827', borderRadius: 24, padding: 36, width: '100%', maxWidth: 440, border: '1px solid #1f2937', boxShadow: '0 25px 50px -12px rgba(0, 0, 0, 0.7)' }}>
          <div style={{ fontWeight: 900, fontSize: 26, color: '#38bdf8', marginBottom: 6, textAlign: 'center', letterSpacing: '-0.025em' }}>AdhereTwin</div>
          <div style={{ fontSize: 14, color: '#9ca3af', marginBottom: 28, textAlign: 'center', lineHeight: 1.5 }}>Enter your Patient ID to access your digital biometric twin</div>
          ...
          <input 
            ...
            style={{ width: '100%', background: '#030712', border: '1px solid #374151', borderRadius: 14, padding: '16px', color: '#f8fafc', fontSize: 15, marginBottom: 20, outline: 'none', transition: 'border-color 0.2s' }} 
          />
          <button 
            onClick={() => { setPatientId(inputId); load(inputId); }} 
            disabled={!inputId.trim() || loading} 
            style={{ width: '100%', background: '#0284c7', color: 'white', border: 'none', borderRadius: 14, padding: '16px', fontWeight: 800, fontSize: 16, cursor: 'pointer', transition: 'background 0.2s', boxShadow: '0 4px 12px rgba(2, 132, 199, 0.3)' }}
          >
  ...
  return (
    <main style={{ minHeight: '100vh', background: '#090d16', color: '#f8fafc', fontFamily: 'system-ui', paddingBottom: 80 }}>
      {/* Header Banner */}
      <header style={{ background: 'linear-gradient(135deg, #0f172a 0%, #1e3a8a 100%)', padding: '32px 20px 40px 20px', borderBottomLeftRadius: 32, borderBottomRightRadius: 32, boxShadow: '0 15px 30px -10px rgba(0,0,0,0.5)' }}>
        <div style={{ maxWidth: 640, margin: '0 auto', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <div>
            <div style={{ fontSize: 12, color: '#7dd3fc', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.08em', marginBottom: 4 }}>Patient Dashboard</div>
            <div style={{ fontSize: 28, fontWeight: 900, color: 'white', letterSpacing: '-0.02em' }}>{data.patient.name}</div>
          </div>
          <div style={{ textAlign: 'right', background: 'rgba(15, 23, 42, 0.65)', padding: '12px 18px', borderRadius: 16, border: '1px solid rgba(255,255,255,0.1)', backdropFilter: 'blur(8px)' }}>
            <div style={{ fontSize: 30, fontWeight: 900, color: data.adherenceRate < 50 ? '#f87171' : data.adherenceRate < 80 ? '#fb923c' : '#34d399', lineHeight: 1 }}>
              {data.adherenceRate}%
            </div>
      ...
      <div style={{ maxWidth: 640, margin: '0 auto', padding: '0 20px' }}>
        ...
        {data.adherenceRate === 100 ? (
          <div style={{ background: '#064e3b', borderRadius: 20, padding: 22, border: '1px solid #059669', marginTop: -20, marginBottom: 24, boxShadow: '0 10px 25px -5px rgba(5, 150, 105, 0.2)' }}>
          ...
          <div style={{ background: '#451a03', borderRadius: 20, padding: 22, border: '1px solid #ea580c', marginTop: -20, marginBottom: 24, boxShadow: '0 10px 25px -5px rgba(234, 88, 12, 0.2)' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
          {data.medications.map((med: any) => {
            ...
            return (
              <div key={med.id} style={{ background: isMissed ? '#2a1215' : '#111827', border: `1px solid ${isMissed ? '#991b1b' : '#1f2937'}`, borderRadius: 20, padding: 22, boxShadow: '0 4px 6px -1px rgba(0,0,0,0.1)', transition: 'all 0.2s' }}>
                ...
                <div style={{ display: 'flex', gap: 12 }}>
                  <button 
                    ...
                    style={{ flex: 1, background: '#059669', color: 'white', border: 'none', borderRadius: 14, padding: '14px 0', fontWeight: 800, fontSize: 15, cursor: 'pointer', transition: 'background 0.2s', boxShadow: '0 4px 12px rgba(5, 150, 105, 0.2)' }}
                  >
                  ...
                  <button 
                    ...
                    disabled={logging === med.id} 
                    style={{ flex: 1, background: '#111827', color: '#f87171', border: '1px solid #7f1d1d', borderRadius: 14, padding: '14px 0', fontWeight: 800, fontSize: 15, cursor: 'pointer', transition: 'background 0.2s' }}
                  >
